r"""Step10: Deeper MLP backbone (3-layer) + dropout regularization.

Key idea: The baseline uses a 2-layer backbone. A 3-layer backbone with dropout
can capture more complex feature interactions while regularization prevents overfitting
on the small 617-sample target training set.
"""
...
from train_balanced_sampling_sep_mlp_shared_calib_step5a_msselect_opt import (
    ...
    report_graph_cache_coverage, compute_src_loss_weight, validate_cell,
)


class DeeperCalibRegressor(nn.Module):
    """3-layer backbone with residual connection + domain calibration heads."""
    def __init__(self, in_dim, design_dim, hid=256, dropout=0.1):
        super().__init__()
        self.fc1 = nn.Linear(in_dim + design_dim, hid)
    ...
    def forward(self, x, z, node="tgt"):
        ...
        h = th.cat([x, z], dim=1)
        h = self.drop(th.relu(self.fc1(h)))
        h_res = h
        h = self.drop(th.relu(self.fc2(h)))
        h = self.drop(th.relu(self.fc3(h))) + h_res  # residual
...
@th.no_grad()
def validate_deeper(val_dl, enc, model, device, design_dim, *, z_dict=None,
                    graph_cache=None, dedup=False, use_amp=False):
    enc.eval(); model.eval()
    loss_fn = nn.MSELoss()
    ...
    return total_loss / max(1, total_n), r2_score.compute().item() if total_n > 0 else 0.0


def train_step10(options, seed):
    ...
    dropout = getattr(options, "mlp_dropout", 0.1)  # default 0.1 for this script
    ...
    model = DeeperCalibRegressor(in_dim=options.in_dim, design_dim=design_dim, hid=hgat_hid, dropout=dropout).to(device)
    print(f"[Step10] DeeperCalibRegressor: 3-layer backbone, dropout={dropout}")
    ...
    for epoch in range(options.num_epoch):
        ...
        val_loss, val_r2 = validate_deeper(val_dl, enc, model, device, design_dim,
        ...
        if better_r2 or tie_better:
            ...
            th.save({"enc": enc.state_dict(), "model": model.state_dict(),
                      ...
                      "script": "step10_deeper_mlp"}, best_ckpt_path)
...
if __name__ == "__main__":
    ...
    with tee.StdoutTee(stdout_f), tee.StderrTee(stderr_f), tee.StdoutTee(copilot_log_f), tee.StderrTee(copilot_log_f):
        run_train_and_report_test(train_step10, options, seed, script_name=__file__)