r"""Step12: Huber loss + early stopping.

Key idea: MSE is sensitive to outliers in delay values. Huber loss (smooth L1)
is more robust. Combined with early stopping to prevent overfitting on the
small target training set.
"""
...
def train_step12(options, seed):
    ...
    df_src, df_tgt_train, df_tgt_val = obj.get("src_df"), obj.get("tgt_train_df"), obj.get("tgt_val_df")
    ...
    report_graph_cache_coverage("src", graph_cache_src, list(ds_src.cts))
    report_graph_cache_coverage("tgt", graph_cache_tgt, list(ds_tgt.cts) + list(val_ds.cts))
    ...
    huber_delta = 1.0  # in normalized y-space; ~73.5 ps in real space
    loss_fn = nn.HuberLoss(delta=huber_delta)
    print(f"[Step12] HuberLoss delta={huber_delta}, early_stop_patience={options.early_stop_patience}")
    ...
    patience_counter = 0
    patience = max(0, options.early_stop_patience) if options.early_stop_patience > 0 else 50
    min_delta = options.early_stop_min_delta if options.early_stop_min_delta > 0 else 1e-4
    ...
    for epoch in range(options.num_epoch):
        ...
        src_w = compute_src_loss_weight(epoch, options)
        ...
        val_huber, val_mse, val_r2 = validate_huber(val_dl, enc, model, device, design_dim,
            huber_delta, z_dict=z_dict_tgt, graph_cache=graph_cache_tgt,
            dedup=options.dedup_z, use_amp=use_amp)

        print(f"e{epoch}, train_loss:{train_loss:.4f}, r2:{train_r2:.3f}, "
              f"val_huber:{val_huber:.4f}, val_mse:{val_mse:.4f}, val_r2:{val_r2:.3f}, src_w:{src_w:.4f}")

        better_r2 = val_r2 > (best_val + CKPT_R2_TIE_EPS_DEFAULT)
        tie_better = abs(val_r2 - best_val) <= CKPT_R2_TIE_EPS_DEFAULT and val_mse < best_val_loss
        if better_r2 or tie_better:
            best_val, best_epoch, best_val_loss = val_r2, epoch + 1, val_mse
            patience_counter = 0
            ...
            th.save({"enc": enc.state_dict(), "model": model.state_dict(),
                      ...
                      "script": "step12_huber_earlystop"}, best_ckpt_path)
            print("Model successfully saved")
        else:
            patience_counter += 1
            if patience_counter >= patience:
                print(f"[EarlyStop] No improvement for {patience} epochs. Stopping at epoch {epoch}.")
                break

    if best_epoch > 0:
        print(f"[Best] epoch:{best_epoch}, val_r2:{best_val:.4f}, val_mse:{best_val_loss:.6f}, ckpt:{best_ckpt_path}")
...
if __name__ == "__main__":
    ...
    with tee.StdoutTee(stdout_f), tee.StderrTee(stderr_f), tee.StdoutTee(copilot_log_f), tee.StderrTee(copilot_log_f):
        run_train_and_report_test(train_step12, options, seed, script_name=__file__)