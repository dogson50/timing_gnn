r"""Step14: Step5a_feat + Pseudo-label self-training (69 unlabeled target samples).
...
class WeightedDataset(TorchDataset):
    """Attach a constant sample weight to each item of a base dataset."""
    def __init__(self, base_ds, sample_weight=1.0):
...
def select_high_confidence_pseudo(x_unlabeled_norm, preds_norm, cts, x_labeled_norm,
    ...
    target samples. Smaller nearest-neighbor distance -> higher confidence.
    """
    ...
    keep_n = min(n_unlabeled, max(1, keep_n))

    # Keep all when ratio asks for all.
    if keep_n >= n_unlabeled:
        idx = np.arange(n_unlabeled, dtype=np.int64)
        dmin = np.zeros((n_unlabeled,), dtype=np.float32)
    # Fallback: no labeled target feature available, keep first keep_n.
    elif x_labeled_norm is None or len(x_labeled_norm) == 0:
        idx = np.arange(keep_n, dtype=np.int64)
        dmin = np.zeros((n_unlabeled,), dtype=np.float32)
    else:
        # n_unlabeled is small (69 in current setup), brute-force distance is cheap.
        diff = x_unlabeled_norm[:, None, :] - x_labeled_norm[None, :, :]
        d2 = np.sum(diff * diff, axis=2)
        dmin = np.sqrt(np.min(d2, axis=1)).astype(np.float32)
        order = np.argsort(dmin)
        # Tie-aware cutoff: if many samples share the same confidence score at the
        # boundary, keep all of them to avoid arbitrary truncation.
        cutoff = float(dmin[order[keep_n - 1]])
        idx = np.where(dmin <= cutoff + 1e-12)[0].astype(np.int64)
...
def train_step14(options, seed):
    ...
    print(f"[Step14] Unlabeled target samples: {len(df_tgt_unlabeled) if has_unlabeled else 0}")
    ...
    # Self-training rounds / pseudo-label controls
    ...
    print(
        f"[Step14] Settings: rounds={num_rounds}, keep_ratio={pseudo_keep_ratio:.2f}, "
    ...
    for rnd in range(num_rounds):
        print(f"\n{'='*60}")
        print(f"[Step14] Self-training round {rnd + 1}/{num_rounds}")
        print(f"{'='*60}")
        ...
        # Combine labeled + pseudo-labeled (with per-sample target weights)
        ds_tgt_w = WeightedDataset(ds_tgt, sample_weight=1.0)
        if pseudo_label_data is not None:
            pl_ds = WeightedDataset(PseudoLabelDataset(*pseudo_label_data), sample_weight=pseudo_loss_weight)
            combined_tgt = ConcatDataset([ds_tgt_w, pl_ds])
            print(
                f"[Step14] Training with {len(ds_tgt)} labeled + {len(pl_ds)} pseudo-labeled = "
                f"{len(combined_tgt)} target samples"
            )
        else:
            combined_tgt = ds_tgt_w
            print(f"[Step14] Training with {len(ds_tgt)} labeled target samples")
        ...
        if rnd > 0 and os.path.exists(round_best_ckpt_path):
            ...
            print(f"[Step14] Warm-started from round {rnd} best checkpoint")
        ...
        for epoch in range(epochs_per_round):
            ...
            if better_r2 or tie_better:
                ...
                th.save({"enc": enc.state_dict(), "model": model.state_dict(),
                          ...
                          "round": rnd + 1, "script": "step14_pseudo_label_feat"}, round_best_ckpt_path)
                if val_r2 > global_best_val + CKPT_R2_TIE_EPS_DEFAULT or \
                   (abs(val_r2 - global_best_val) <= CKPT_R2_TIE_EPS_DEFAULT and val_loss < global_best_loss):
                    ...
                    th.save({"enc": enc.state_dict(), "model": model.state_dict(),
                              "design_dim": design_dim, "hgat_hid": hgat_hid, "hgat_heads": hgat_heads,
                              "scaler_stats": scaler_stats, "y_scaler": y_scaler,
                              "epoch": global_ep + 1, "best_val_r2": global_best_val, "best_val_loss": global_best_loss,
                              "round": rnd + 1, "script": "step14_pseudo_label_feat"}, best_ckpt_path)
        ...
        if has_unlabeled and rnd < num_rounds - 1:
            ...
            print(
                f"[Step14] Generated {len(preds_norm)} pseudo-labels, selected {len(x_pl)} high-confidence "
                f"for next round, {d_msg}"
            )
