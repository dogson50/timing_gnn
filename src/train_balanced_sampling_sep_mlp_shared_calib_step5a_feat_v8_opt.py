...
class CellDelayDataset(TorchDataset):
    def __init__(
        ...
        if cell_to_id is None:
            cell_to_id = {"__UNK__": 0}
        self.cell_to_id = dict(cell_to_id)
        self.cell_ids = np.array([int(self.cell_to_id.get(str(c), 0)) for c in self.cts], dtype=np.int64)
...
def build_cell_vocab(*dfs):
    cell_set = set()
    for df in dfs:
        if df is None or len(df) == 0:
            continue
        if "cell_type" in df.columns:
            vals = df["cell_type"].fillna("__UNK__").astype(str).unique().tolist()
            cell_set.update(vals)
    cell_list = sorted(cell_set)
    cell_to_id = {"__UNK__": 0}
    for c in cell_list:
        if c == "__UNK__":
            continue
        cell_to_id[c] = len(cell_to_id)
    return cell_to_id
...
class SharedCalibRegressor(nn.Module):
    def __init__(
        ...
        graph_adapter=False,
        graph_adapter_hidden=32,
        graph_adapter_dropout=0.0,
        graph_adapter_scale=1.0,
        topo_fusion_gate=False,
        topo_fusion_gate_hidden=0,
        topo_fusion_gate_dropout=0.0,
        topo_fusion_gate_prior=1.0,
        cell_variant_residual=False,
        cell_variant_dim=8,
        cell_variant_hidden=32,
        cell_variant_scale=1.0,
    ):
        super().__init__()
        self.design_dim = int(design_dim)
        self.use_graph_adapter = bool(graph_adapter)
        self.graph_adapter_scale = max(0.0, float(graph_adapter_scale))
        if self.use_graph_adapter:
            ga_hid = max(4, int(graph_adapter_hidden))
            ga_drop = max(0.0, float(graph_adapter_dropout))
            self.graph_adapter = nn.Sequential(
                nn.Linear(self.design_dim, ga_hid),
                nn.ReLU(),
                nn.Dropout(ga_drop),
                nn.Linear(ga_hid, self.design_dim),
            )
        else:
        ...
        self.use_topo_fusion_gate = bool(topo_fusion_gate)
        self.topo_fusion_gate_prior = max(0.0, float(topo_fusion_gate_prior))
        self.use_cell_variant_residual = bool(cell_variant_residual)
        self.cell_variant_scale = max(0.0, float(cell_variant_scale))
        if self.use_topology_expert:
            ...
            if self.use_topo_fusion_gate:
                gate_hid = int(topo_fusion_gate_hidden)
                gate_drop = max(0.0, float(topo_fusion_gate_dropout))
                if gate_hid > 0:
                    self.topo_fusion_gate = nn.Sequential(
                        nn.Linear(topo_dim, gate_hid),
                        nn.ReLU(),
                        nn.Dropout(gate_drop),
                        nn.Linear(gate_hid, 1),
                    )
                else:
                    self.topo_fusion_gate = nn.Linear(topo_dim, 1)
            else:
                self.topo_fusion_gate = None
            ...
            if self.use_cell_variant_residual:
                cell_dim = max(2, int(cell_variant_dim))
                cell_hid = max(4, int(cell_variant_hidden))
                self.cell_emb = nn.Embedding(self.num_cells, cell_dim)
                cell_in_dim = pred_hid + topo_dim + cell_dim
                self.cell_res_tgt = nn.Sequential(
                    nn.Linear(cell_in_dim, cell_hid),
                    nn.ReLU(),
                    nn.Linear(cell_hid, 1),
                )
                self.cell_res_src = nn.Sequential(
                    nn.Linear(cell_in_dim, cell_hid),
                    nn.ReLU(),
                    nn.Linear(cell_hid, 1),
                )
                ...
                self.cell_emb = None
                self.cell_res_tgt = None
                self.cell_res_src = None
        else:
            self.topo_emb = None
            self.topo_fusion_gate = None
            self.topo_res_tgt = None
    ...
    def _adapt_z(self, z, topo_ids=None):
        if z.dim() == 1:
            z = z.unsqueeze(0)
        if self.graph_adapter is not None:
            z = z + self.graph_adapter_scale * self.graph_adapter(z)
        if self.topo_fusion_gate is not None and topo_ids is not None:
            topo_ids = topo_ids.to(device=z.device, dtype=th.long).clamp_(0, self.num_topologies - 1)
            topo_feat = self.topo_emb(topo_ids)
            gate = th.sigmoid(self.topo_fusion_gate(topo_feat))
            z = z * (self.topo_fusion_gate_prior * gate)
        return z

    def encode_hidden(self, x, z, topo_ids=None):
        z = self._adapt_z(z, topo_ids=topo_ids)
    ...
    def _apply_cell_variant_residual(self, h_topo, topo_ids, cell_ids, node):
        if (not self.use_cell_variant_residual) or topo_ids is None or cell_ids is None or self.cell_emb is None:
            return th.zeros((h_topo.shape[0],), device=h_topo.device, dtype=h_topo.dtype)
        topo_ids = topo_ids.to(device=h_topo.device, dtype=th.long).clamp_(0, self.num_topologies - 1)
        cell_ids = cell_ids.to(device=h_topo.device, dtype=th.long).clamp_(0, self.num_cells - 1)
        topo_feat = self.topo_emb(topo_ids)
        cell_feat = self.cell_emb(cell_ids)
        cell_in = th.cat([h_topo, topo_feat, cell_feat], dim=1)
        if node == "tgt":
            res = self.cell_res_tgt(cell_in).squeeze(-1)
        elif node == "src":
            res = self.cell_res_src(cell_in).squeeze(-1)
        else:
            raise ValueError(f"Unknown node type: {node}")
        return self.cell_variant_scale * res

    def forward_with_aux(self, x, z, node="tgt", topo_ids=None, cell_ids=None):
        aux = self.encode_hidden(x, z, topo_ids=topo_ids)
        ...
        pred = pred + self._apply_cell_variant_residual(h_topo, topo_ids, cell_ids, node)
        return pred, aux

    def forward(self, x, z, node="tgt", topo_ids=None, cell_ids=None):
        pred, _ = self.forward_with_aux(x, z, node=node, topo_ids=topo_ids, cell_ids=cell_ids)
        return pred
...
def validate_cell(
    ...
    *,
    ...
    for batch in val_dl:
        if len(batch) == 9:
            xb, yb, cts, from_pins, to_pins, topo_ids, cell_ids, _, _ = batch
        elif len(batch) == 8:
        ...
        with th.cuda.amp.autocast(enabled=amp_on):
            ...
            pred = model(xb, zb, node="tgt", topo_ids=topo_ids, cell_ids=cell_ids)
...
def train_balanced_sep_mlp_shared_calib(options, seed):
    ...
    cell_to_id = build_cell_vocab(df_src_use, df_tgt_train, df_tgt_val, df_tgt_test)
    ...
    ds_src = CellDelayDataset(
        df_src_use, x_mean, x_std, y_mean, y_std, group_col=tgt_group_col, topo_to_id=topo_to_id, cell_to_id=cell_to_id
    )
    val_ds = CellDelayDataset(
        df_tgt_val, x_mean, x_std, y_mean, y_std, group_col=tgt_group_col, topo_to_id=topo_to_id, cell_to_id=cell_to_id
    )
    has_test_samples = df_tgt_test is not None and len(df_tgt_test) > 0
    test_ds = (
        CellDelayDataset(df_tgt_test, x_mean, x_std, y_mean, y_std, group_col=tgt_group_col, topo_to_id=topo_to_id, cell_to_id=cell_to_id)
        if has_test_samples
        else None
    )
    ...
    def my_collate(batch):
        ...
        if item_len == 9:
            xs, ys, cts, from_pins, to_pins, topo_ids, cell_ids, group_ids, sample_ws = zip(*batch)
        elif item_len == 8:
            xs, ys, cts, from_pins, to_pins, topo_ids, group_ids, sample_ws = zip(*batch)
            cell_ids = tuple([0] * len(xs))
        elif item_len == 7:
        ...
        return (
            ...
            th.tensor(cell_ids, dtype=th.long),
            group_ids,
    ...
    v8_graph_adapter = bool(getattr(options, "v8_graph_adapter", False))
    v8_graph_adapter_hidden = int(getattr(options, "v8_graph_adapter_hidden", 32))
    v8_graph_adapter_dropout = max(0.0, float(getattr(options, "v8_graph_adapter_dropout", 0.0)))
    v8_graph_adapter_scale = max(0.0, float(getattr(options, "v8_graph_adapter_scale", 1.0)))
    v8_topo_fusion_gate = bool(getattr(options, "v8_topo_fusion_gate", False))
    v8_topo_fusion_gate_hidden = int(getattr(options, "v8_topo_fusion_gate_hidden", 0))
    v8_topo_fusion_gate_dropout = max(0.0, float(getattr(options, "v8_topo_fusion_gate_dropout", 0.0)))
    v8_topo_fusion_gate_prior = max(0.0, float(getattr(options, "v8_topo_fusion_gate_prior", 2.0)))
    v8_cell_variant_residual = bool(getattr(options, "v8_cell_variant_residual", False))
    v8_cell_variant_dim = int(getattr(options, "v8_cell_variant_dim", 8))
    v8_cell_variant_hidden = int(getattr(options, "v8_cell_variant_hidden", 32))
    v8_cell_variant_scale = max(0.0, float(getattr(options, "v8_cell_variant_scale", 1.0)))
    num_topologies = max(1, len(topo_to_id))
    num_cells = max(1, len(cell_to_id))
    ...
    print(
        f"[Info] V8 graph adapter: {'enabled' if v8_graph_adapter else 'disabled'}"
        + (f" (hid={v8_graph_adapter_hidden}, dropout={v8_graph_adapter_dropout:.3f}, scale={v8_graph_adapter_scale:.3f})" if v8_graph_adapter else "")
    )
    print(
        f"[Info] V8 topology fusion gate: {'enabled' if (v8_topo_fusion_gate and use_topology_expert) else 'disabled'}"
        + (f" (hid={v8_topo_fusion_gate_hidden}, dropout={v8_topo_fusion_gate_dropout:.3f}, prior={v8_topo_fusion_gate_prior:.3f})" if (v8_topo_fusion_gate and use_topology_expert) else "")
    )
    print(
        f"[Info] V8 cell-variant residual: {'enabled' if (v8_cell_variant_residual and use_topology_expert) else 'disabled'}"
        + (f" (num_cells={num_cells}, dim={v8_cell_variant_dim}, hid={v8_cell_variant_hidden}, scale={v8_cell_variant_scale:.3f})" if (v8_cell_variant_residual and use_topology_expert) else "")
    )
    ...
    model = SharedCalibRegressor(
        ...
        graph_adapter=v8_graph_adapter,
        graph_adapter_hidden=v8_graph_adapter_hidden,
        graph_adapter_dropout=v8_graph_adapter_dropout,
        graph_adapter_scale=v8_graph_adapter_scale,
        topo_fusion_gate=v8_topo_fusion_gate,
        topo_fusion_gate_hidden=v8_topo_fusion_gate_hidden,
        topo_fusion_gate_dropout=v8_topo_fusion_gate_dropout,
        topo_fusion_gate_prior=v8_topo_fusion_gate_prior,
        cell_variant_residual=v8_cell_variant_residual,
        cell_variant_dim=v8_cell_variant_dim,
        cell_variant_hidden=v8_cell_variant_hidden,
        cell_variant_scale=v8_cell_variant_scale,
    ).to(device)
    ...
    for epoch in range(options.num_epoch):
        ...
        for xb_tgt, yb_tgt, cts_tgt, from_pins_tgt, to_pins_tgt, topo_ids_tgt, cell_ids_tgt, _, sample_w_tgt in dl_tgt:
            ...
            cell_ids_tgt = cell_ids_tgt.to(device, non_blocking=True)
            ...
            with th.cuda.amp.autocast(enabled=amp_on):
                ...
                pred_tgt, aux_tgt = model.forward_with_aux(xb_tgt, zb_tgt, node="tgt", topo_ids=topo_ids_tgt, cell_ids=cell_ids_tgt)
            ...
            for _ in range(max(1, options.sample_45_num)):
                try:
                    xb_src, yb_src, cts_src, from_pins_src, to_pins_src, topo_ids_src, cell_ids_src, _, _ = next(dl_src_iter)
                ...
                cell_ids_src = cell_ids_src.to(device, non_blocking=True)
                src_n = len(yb_src)
                with th.cuda.amp.autocast(enabled=amp_on):
                    z_ctx = contextlib.nullcontext() if update_encoder_this_step else th.no_grad()
                    with z_ctx:
                    ...
                    pred_src, aux_src = model.forward_with_aux(xb_src, zb_src, node="src", topo_ids=topo_ids_src, cell_ids=cell_ids_src)
...
def _parse_v7_args(argv):
    ...
    parser.add_argument("--v8_graph_adapter", action="store_true",
                        help="enable a small residual adapter on frozen graph embedding z")
    parser.add_argument("--v8_graph_adapter_hidden", type=int, default=32,
                        help="hidden dim for graph adapter")
    parser.add_argument("--v8_graph_adapter_dropout", type=float, default=0.0,
                        help="dropout inside graph adapter")
    parser.add_argument("--v8_graph_adapter_scale", type=float, default=1.0,
                        help="scale factor for graph adapter residual")
    parser.add_argument("--v8_topo_fusion_gate", action="store_true",
                        help="enable topology-conditioned scalar graph gate")
    parser.add_argument("--v8_topo_fusion_gate_hidden", type=int, default=0,
                        help="hidden dim for topology-conditioned graph gate (0=linear)")
    parser.add_argument("--v8_topo_fusion_gate_dropout", type=float, default=0.0,
                        help="dropout inside topology graph gate")
    parser.add_argument("--v8_topo_fusion_gate_prior", type=float, default=2.0,
                        help="multiplicative prior so sigmoid gate initializes around one effective graph scale")
    parser.add_argument("--v8_cell_variant_residual", action="store_true",
                        help="add cell-variant residual on top of topology residual")
    parser.add_argument("--v8_cell_variant_dim", type=int, default=8,
                        help="embedding dim for cell-type variant residual")
    parser.add_argument("--v8_cell_variant_hidden", type=int, default=32,
                        help="hidden dim for cell-type variant residual")
    parser.add_argument("--v8_cell_variant_scale", type=float, default=1.0,
                        help="scale factor for cell-type variant residual")
