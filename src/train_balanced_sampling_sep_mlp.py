r"""Balanced sampling training for cell delay regression using HGAT embeddings (separate MLP heads)."""
...
class SepCellDelayRegressor(nn.Module):
    ...
    def forward(self, x, z, node="tgt", from_pin_id=None, to_pin_id=None, pol_id=None):
        ...
        if node == "src":
            return self.mlp_src(h).squeeze(-1)
        raise ValueError(f"Unknown node type: {node}")
...
if __name__ == "__main__":
    ...
    with tee.StdoutTee(stdout_f), tee.StderrTee(stderr_f), tee.StdoutTee(copilot_log_f), tee.StderrTee(copilot_log_f):
        run_train_and_report_test(train_balanced_sep_mlp, options, seed, script_name=__file__)