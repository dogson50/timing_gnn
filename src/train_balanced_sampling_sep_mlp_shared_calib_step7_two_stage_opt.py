...
GRAPH_NET_DIM = 7
GRAPH_MOS_DIM = 8
TWO_STAGE_FREEZE_RATIO_DEFAULT = 0.25
...
def train_balanced_sep_mlp_shared_calib(options, seed):
    ...
    two_stage_enable = bool(getattr(options, "two_stage_enable", True)) and (not bool(options.freeze_hgat))
    freeze_epochs_opt = int(getattr(options, "two_stage_freeze_epochs", -1))
    if freeze_epochs_opt >= 0:
        stage1_freeze_epochs = max(0, min(freeze_epochs_opt, int(options.num_epoch)))
    else:
        stage1_freeze_epochs = int(round(float(options.num_epoch) * TWO_STAGE_FREEZE_RATIO_DEFAULT))
        stage1_freeze_epochs = max(1, min(stage1_freeze_epochs, int(options.num_epoch)))
    ...
    if options.freeze_hgat:
        ...
        if two_stage_enable:
            print(
                f"[Info] two_stage_enable=True, stage1 freeze epochs={stage1_freeze_epochs}, "
                f"stage2 epochs={max(0, int(options.num_epoch) - stage1_freeze_epochs)}"
            )
            z_dict_src = precompute_z_from_graph_cache(graph_cache_src, enc)
            z_dict_tgt = precompute_z_from_graph_cache(graph_cache_tgt, enc)
            for p in enc.parameters():
    ...
    stage2_unfrozen = False

    for epoch in range(options.num_epoch):
        if two_stage_enable and (not stage2_unfrozen) and epoch >= stage1_freeze_epochs:
            stage2_unfrozen = True
            z_dict_src = None
            z_dict_tgt = None
            for p in enc.parameters():
            ...
            print(
                f"[Info] Stage2 starts at epoch {epoch + 1}: unfreeze HGAT, "
                f"enc_lr={enc_lr:.3e}, model_lr={options.learning_rate:.3e}"
            )

        if options.freeze_hgat:
            enc.eval()
        elif two_stage_enable and (not stage2_unfrozen):
            enc.eval()
        else:
            enc.train()
        model.train()
        ...
        for xb_tgt, yb_tgt, cts_tgt in dl_tgt:
            ...
            src_n_sum = 0
            z_step_cache = {} if (not options.freeze_hgat and (not two_stage_enable or stage2_unfrozen)) else None
