...
def infer_top_pin_roles(top_pins):
    pins_up = [str(p).upper() for p in (top_pins or [])]
    power_set = {"VDD", "VPWR", "VCC", "VSS", "VGND", "GND", "VNW", "VPB", "VNB"}
    output_name_set = {
        "Y", "Z", "ZN", "Q", "QN", "S", "SO", "CO", "COUT",
        "SUM", "OUT", "X", "XOR", "XNOR",
    }

    signal_pins = [p for p in pins_up if p not in power_set]
    output_pin = None
    for p in signal_pins:
        if p in output_name_set:
            output_pin = p
            break
    if output_pin is None and len(signal_pins) > 0:
        output_pin = signal_pins[-1]

    role = {}
    for p in pins_up:
        if p in power_set:
            role[p] = "power"
        elif output_pin is not None and p == output_pin:
            role[p] = "output"
        else:
            role[p] = "input"
    return role
...
def build_dgl_graph_from_devs_step5(devs, top_pins):
    ...
    top_pin_role = infer_top_pin_roles(top_pins)
    ...
    for name, nid in sorted(nets.items(), key=lambda x: x[1]):
        ...
        role = top_pin_role.get(up, "internal")
        is_input_pin = 1.0 if role == "input" else 0.0
        is_output_pin = 1.0 if role == "output" else 0.0
        is_power_pin = 1.0 if role == "power" else 0.0
