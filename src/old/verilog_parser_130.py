...
with open('../rawdata-130/early_lib.json', 'r') as f:
    cell_lib = json.load(f)

REGISTER_LIST=["dfbbn", "dfbbp", "dfrbp", "dfrtn", "dfrtp", "dfsbp", "dfstp", "dfxbp", "dfxtp", "dlrbn", "dlrbp",
                        "dlrtn", "dlrtp", "dlxbn", "dlxbp", "dlxtn", "dlxtp", "sdfbbn", "sdfbbp", "sdfrbp", "sdfrtn", "sdfrtp",
                        "sdfsbp", "sdfstp", "sdfxbp", "sdfxtp", "sedfxbp", "sedfxtp", "edfxbp", "edfxtp", "sdlclkp"]
...
class Parser:
    ...
    def parse_path(self, path_text, extract_reg=False):
        """
        ...
        for line in lines:
            ...
            # print(context)
            # if len(context) <= 3:
            #     print(f'context error!: {context} {line} {lines}')
            #     exit()
        ...
        return startpoint, endpoint, path, nets, required_time, arrival_time, (reg_start, reg_end)
    ...
    def parse_postoptReport(self, fname):
        ...
        for i, block in enumerate(blocks):
            ...
            startpoint, endpoint, path, nets, required_time, arrival_time, _ = self.parse_path(block)
        ...
        print('#paths: {}, #critical: {}, #endpoint: {}'.format(len(all_paths), len(post_criticals), len(self.endpoints)))
        # return critical_ends

    def parse_preoptReport(self, fname, extract_reg=False):
        r"""
        ...
        :param fname: str
                    the path of the report file
        :param extract_reg: boolean
                    output the register (startpoint / endpoint of the timing paths)
        :return:
        ...
        for i, block in enumerate(blocks):
            ...
            startpoint, endpoint, path, nets, required_time, arrival_time, reg = self.parse_path(block)
            if extract_reg:
                reg_all.append(reg[0])
                reg_all.append(reg[1])
                # print(f'startpoint: {startpoint} start reg cell: {reg[0]}, end reg cell: {reg[1]}')
            if path_state == 'VIOLATED':
        ...
        if extract_reg:
            reg_all = set(reg_all)
            print(f'All the registers: {reg_all}')
        return self.timing_paths, reg_all
        ...
        for sentence in module.children():
            if type(sentence) == Decl:
                for decl in sentence.children():
                    name = decl.name
                    # ignore the UNCONNECTED wire
                    if "UNCONNECTED" in name:
                        continue
                    # print(f'In decl, name: {name}')
                    ...
                    if decl.dimensions is None and decl.width is None:
                        ...
                        if high_bit < low_bit:
                            ...
                            high_bit = low_bit
                            low_bit = temp
                        # print(f'In decl, dimension not None, name {name}, high {high_bit} low {low_bit}')
                        ...
                        if high_bit < low_bit:
                            ...
                            high_bit = low_bit
                            low_bit = temp
                        # print(f'In decl, width not None, name {name}, high {high_bit} low {low_bit}')
            ...
            # record the equavalent wires (assign)
                ...
                # print(f'rt value: {rt_value}')
                if "UNCONNECTED" in rt_value or "UNCONNECTED" in lf_value:
                    print(f'left value: {lf_value} rt value: {rt_value}')
                    exit()
    ...
    def parse_io2arg(self, portlist, wires, father_wires, father_io2arg):
        ...
        for p in portlist:
            ...
            # ignore the UNCONNECTED port
            if len(port_info.arg_list) == 0:
                continue
            ...
            if width == 1:
                ...
                if father_io2arg is not None and \
                    ...
                    # print(f'portname {portname} arg_name: {arg_name} father io2arg: {father_io2arg[arg_name][1]}')
                    io2arg[portname] = \
                        (wire_type, father_io2arg[arg_name][1], father_io2arg[arg_name][2] + 1)
            else:
                indx2 = 0
                for i in range(high_bit, low_bit - 1, -1):
                    # if indx2 >= len(port_info.arg_list):
                    #     print(f'port info arg list out of range!')
                    #     print(f'indx2: {indx2}')
                    #     print(f'wires: {wires[portname]}')
                    #     print(f'port info: {port_info.portname} {port_info.arg_list}')
                    #     exit()
    ...
    def parse_module(self, module_name, instance_name, portlist,
        ...
        if module_name == self.top_module:
            ...
            # print(f'port list: {[(p.portname, p.argname) for p in portlist[:2]]}')
            io2arg = self.parse_io2arg(portlist, wires, father_wires, father_io2arg)
            # print(f'io2arg: {io2arg}')
            # exit()
            if call_path == '':
        ...
        for item in module.items:
            ...
            if item_cell in self.modulelist:
                ...
                # TODO: handle the SRAM cell in 130nm
        ...
        for p in portlist:
            ...
            for i, netname in enumerate(port_info.arg_list):
                ...
                if call_path == '':
                    ...
                    pinname = '{}/{}/{}[{}]'.format(call_path, instance_name, port_info.portname,
                                                    width - 1 - i) if width > 1 \
    ...
    def parse_cell(self, cell_name, instance_name, portlist, io2arg, call_path):
        ...
        cell_type = cell_name[17:]  # The prefix is "sky130_fd_sc_hd__"
        cell_type = cell_type.split('_')[0]
        # print(f'cell type: {cell_type}')
        # cell_type = cell_name[:idx.start()]  # e.g. INV
        ...
        for port in portlist:
            ...
            # Skip the "UNCONNECTED" pins
            port_netname = port_info.arg_list[0]
            if "UNCONNECTED" in port_netname:
                continue
        ...
        for port in port_info_list:
            ...
            # print(f'port pinname: {port_pinname} port netname: {port_netname}')
            # get the location of the current cell instance
            ...
            # if "r_RStat" in port_netname and "miim1" in port_pinname:
            #     print(f'Check netname: {port_netname}')
            #     import pdb
            #     pdb.set_trace()
            # if 'ethreg1/MIICOMMAND1/DataOut_reg[0]/Q' in port_pinname:
            #     print(f'Check port pinname: {port_pinname}')
            #     import pdb
            #     pdb.set_trace()
            ...
            if port.ptype == 'fanout':
                ...
                try:
                    ...
                    c = cell_info_map[cell_name]['pin_info'][port.portname]['capacitance']
                    print(f'EMPTY capacitance in fan in sink pin! cell name: {cell_name} port netname: {port_netname} port pinname: {port_pinname} cap: {c}')
                    exit()
                    cap = 0.
                ...
                try:
                    ...
                    c = cell_info_map[cell_name]['pin_info'][port.portname]['capacitance']
                    print(f'EMPTY capacitance in CLK! cell name: {cell_name} port netname: {port_netname} port pinname: {port_pinname} cap: {c}')
                    exit()
                    cap = 0.
                    # exit()
                self.nets[port_netname].total_output_cap += cap
            ...
            node = (port_pinname,
                    {'net': port_netname,
                     ...
                     'position': pin_location,
                     'DFF': 'DFF' in cell_name})
            if "UNCONNECTED" in port_netname or "UNCONNECTED" in cell_name:
                raise ValueError(f"UNCONNECTED NETS FOUND: port netname: {port_netname} port cell name: {cell_name}")
            sub_nodes.append(node)
    ...
    def check_path(self, graph, path):
        ...
        for nd in path[1:]:
            ...
            pre_nd = nd
        if not flag:
            print(f'Check path failed: {path}')
    ...
    def parse_netlist(self, fname):
        ...
        for net, net_info in self.nets.items():
            ...
            for sink_pin in sink_pins:
                assert "UNCONNECTED" not in net, "UNCONNECTED NETS FOUND!"
                edges.append(
                    (drive_pin,
                     sink_pin,
        ...
        c = None
        try:
            c = list(nx.find_cycle(graph, orientation="original"))
        except:
            print("FOUND NO CYCLES IN THE GRAPH")
        if c is not None:
            raise ValueError("WRONG NETLIST GRAPH PARSED")
        # exit()
    ...
    def parse(self, data_dir):
        ...
        pre_pin_loc_file = os.path.join(data_dir,
                                        'positions/pin_bin.txt')  # a file that contains the location of each instance
        ...
        timing_paths, reg_all = self.parse_preoptReport(preopt_report_path, extract_reg=True)
        reg_all_save_path = os.path.join(self.data_path, 'reg_all.txt')
        with open(reg_all_save_path, 'w') as f:
            for reg in reg_all:
                if reg is None:
                    continue
                ctype = cell_info_map[reg]['type']
                if ctype not in REGISTER_LIST:
                    raise ValueError("Miss register types")
                f.write(f"Cell name {reg} cell type {ctype} \n")
    ...
    def cal_topo_level(self, PIs: set, POs: set, PO2pathID: dict):
        ...
        # reverse visit the topo levels, and remove the visited nodes in each level
