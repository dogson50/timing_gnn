...
def extract_feat(loader, device, model, cnn, beta, options, node='7'):
    ...
    with th.no_grad():
        ...
        print('validate:')
        case_idx = 0
        all_dataset_feat_node = []
        all_dataset_feat_others = []
        for path_dataset, graph, path2level, path2endpoint, topo_levels, cnn_inputs, path_masks in loader:
            ...
            count_target = 0
            label_hats = None
            all_feat_node, all_feat_others = None, None
            target_list = []
            ...
            for path_ids in path_loader:
                ...
                for level_id, level in enumerate(topo_levels):
                    ...
                    cur_feat_node = F.normalize(cur_feat_node, dim=1)
                    if all_feat_node is None:
                        all_feat_node = cur_feat_node
                    else:
                        all_feat_node = th.cat((all_feat_node, cur_feat_node), dim=0)
                    if all_feat_others is None:
                        all_feat_others = cur_feat_others
                    else:
                        all_feat_others = th.cat((all_feat_others, cur_feat_others), dim=0)
            all_dataset_feat_node.append(all_feat_node)
            all_dataset_feat_others.append(all_feat_others)
    return all_dataset_feat_node, all_dataset_feat_others
...
def main(options, seed):
    ...
    all_feat_7_node, all_feat_7_others = extract_feat(val_dataset, device, model, cnn, beta, options, node='7')
    all_feat_130_node, all_feat_130_others  = extract_feat(train_dataset_130, device, model, cnn, beta, options, node='130')
    names_7 = ['arm9', 'chacha', 'hwacha', 'or1200', 'sha3', 'rocket', 'smallboom', 'steelcore']
    names_130 = ['jpeg', 'linkruncca', 'spiMaster', 'usbf_device']
    tsne_path = '/research/d4/gds/xyzhang21/Timing/DAC23-Timing-Prediction/src/tsne/disentangle/node'
    tsne(all_feat_7_node, all_feat_130_node, names_7, names_130, load=False, save=True, sample=1000, tsne_path=tsne_path)
    exit()
    tsne(all_feat_7_others, all_feat_130_others, names_7, names_130, load=False, save=True, sample=1000)
...
def draw_separate(all_feat, all_label, names_7, names_130, size_7, size_130, tsne_path):
    # draw all in one fig
    colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'w', 'orange', 'purple', 'pink', 'brown']
    save_path = os.path.join(tsne_path, 'all_tsne.png')
    plt.figure(figsize=(10, 10))
    for i in range(all_feat.shape[0]):
        plt.scatter(all_feat[i, 0], all_feat[i, 1], color=colors[int(all_label[i])], s=0.5)
    all_names = names_7.copy()
    all_names.extend(names_130)
    print(f'names 7: {names_7}')
    print(f'names 130: {names_130}')
    print(f'all names: {all_names}')
    # Create a legend
    for i, n in enumerate(all_names):  # Replace 10 with the number of your classes
        plt.scatter([], [], color=colors[i], label=n)
...
def tsne(all_feat_7, all_feat_130, names_7, names_130, load=False, save=False, sample=500, separate=False, tsne_path=None):
    ...
    print(f'all label shape: {all_label_np.shape}')
