...
def init_model(options):
    ...
    model = SepBayesModel(gnn, fcn, mlp_7, mlp_130)
        ...
        for path_dataset, graph, path2level, path2endpoint, topo_levels, cnn_inputs, path_masks in loader:
            ...
            for path_ids in path_loader:
                ...
                for level_id, level in enumerate(topo_levels):
                    ...
                    output = model(graph, nodes, eids, targets, level_id, path_map, training=False, sampling=options.sampling)

                    if len(paths) == 0:
                        continue

                    cur_label_hats, _ = output
...
def path_batch_forward(path_ids, path2level, path2endpoint, topo_levels, feat_map, model, graph, path_masks, node='7',
    ...
    # print(f'kl all shape: {kl_all.shape}')
    # exit()
    return label_hats, arrival_time, kl_all
...
def contrastive_loss(node_feat_7, node_feat_130, temp, device):
    ...
    all_node_feat = th.cat((node_feat_7, node_feat_130), dim=0)
...
def train(options, seed):
    ...
    for epoch in range(options.num_epoch):
        ...
        for bidx in range(num_batch_7):
            ...
            label_hats_7, arrival_time_7, kl_node_7 = path_batch_forward(path_ids_7, path2level_7,
                                                                         path2endpoint_7,topo_levels_7,
                                                                         feat_map_7, model, graph_7,
                                                                         path_masks_7, kl=options.kl,
                                                                         sampling=options.sampling,
                                                                         gaussian_prior=options.gaussian_prior)
            # sample from 130nm data
            label_hats_130, arrival_time_130, kl_node_130 = [], [], []
            ...
            for i in sampled_130_idx:
                ...
                try:
                    ...
                    path_ids_130_ = next(path_loader_iter_130_)
                label_hats_130_, arrival_time_130_, kl_node_130_ = path_batch_forward(path_ids_130_,
                                                                                      path2level_130_,
                                                                                      path2endpoint_130_,
                                                                                      topo_levels_130_,
                                                                                      feat_map_130_,
            ...
            kl_loss = kl_7 + kl_130
            total_loss = total_loss + options.weight_kl * kl_loss
            ...
            print("e{}, sampled 130 designs: {}, b{}/{}, total loss:{:.2f}, 7nm reg loss: {:.2f}, 130nm reg loss: {}, "
                  "kl 7nm: {:.2f}, kl 130nm: {:.2f}  7nm r2:{:.2f}, 130nm r2: {}, batch time: {:.2f}".format(epoch,
                                                                                designs_info, bidx, num_batch_7,
                                                                                total_loss.item(), train_loss_7.item(),
                                                                                loss_130_msg, kl_130.item(), kl_7.item(),
