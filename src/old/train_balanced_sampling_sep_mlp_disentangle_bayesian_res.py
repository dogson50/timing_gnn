...
def init_model(options):
    ...
    mlp_7 = MlpBayesRes(
        ...
        K=options.mc_num,
        momentum=options.ema_momentum,
        fix_alpha=options.fix_alpha,
        alpha=options.res_alpha
    )
    mlp_130 = MlpBayesRes(
        in_dim=mlp_dim,
        # in_dim = options.out_dim,
    ...
    model = SepBayesDisentangleModel(gnn, fcn, mlp_node, mlp_others, mlp_7, mlp_130)
        ...
        for path_dataset, graph, path2level, path2endpoint, topo_levels, cnn_inputs, path_masks in loader:
            ...
            for path_ids in path_loader:
                ...
                for level_id, level in enumerate(topo_levels):
                    ...
                    output = model(graph, nodes, eids, targets, level_id, path_map, training=False,
                                   sampling=options.sampling)

                    if len(paths) == 0:
                        continue

                    (cur_label_hats, _), _, _ = output
...
def path_batch_forward(path_ids, path2level, path2endpoint, topo_levels, feat_map, model, graph, path_masks, node='7',
                       kl=False, sampling=False, gaussian_prior=False):
    ...
    count_target = 0
    label_hats, kl_all, feat_node, feat_others = None, None, None, None
    target_list = []
    for level_id, level in enumerate(topo_levels):
        nodes, eids = level[:2]
        if len(level) == 2:
        ...
        output = model(graph, nodes, eids, targets, level_id, path_map, node=node, training=True,
                       kl=kl, sampling=sampling, gaussian_prior=gaussian_prior)
        ...
        if label_hats is None:
            ...
            label_hats = th.cat((label_hats, cur_label_hats), dim=0)
        if kl_all is None:
            kl_all = cur_kl.unsqueeze(1)
        else:
            kl_all = th.cat((kl_all, cur_kl.unsqueeze(1)), dim=0)
    ...
    # labels = graph.ndata['label'][target_list].squeeze()
    # print(f'label hats shape: {label_hats.shape}')
    arrival_time = graph.ndata['arrival_time'][target_list].squeeze()
    return label_hats, arrival_time, kl_all, feat_node, feat_others
...
def train(options, seed):
    ...
    for epoch in range(options.num_epoch):
        ...
        if epoch >= options.kl_start_ep:
            kl_flag = True
            print(f'adding kl loss!')
        ...
        for bidx in range(num_batch_7):
            ...
            label_hats_7, arrival_time_7, kl_node_7, feat_node_7, feat_others_7 = path_batch_forward(path_ids_7,
                                                                                                     path2level_7,
                                                                                                     path2endpoint_7,
                                                                                                     topo_levels_7,
                                                                                                     feat_map_7, model,
                                                                                                     ...
                                                                                                     kl=options.kl,
                                                                                                     sampling=options.sampling,
                                                                                                     gaussian_prior=options.gaussian_prior)
            # sample from 130nm data
            label_hats_130, arrival_time_130, kl_node_130, feat_node_130, feat_others_130 = [], [], [], [], []
            ...
            for i in sampled_130_idx:
                ...
                try:
                    ...
                    path_ids_130_ = next(path_loader_iter_130_)
                label_hats_130_, arrival_time_130_, kl_node_130_, feat_node_130_, feat_others_130_ = path_batch_forward(
                    path_ids_130_,
                    path2level_130_,
                    path2endpoint_130_,
                    topo_levels_130_,
            ...
            kl_node_130 = th.cat(kl_node_130)
            kl_130 = torch.sum(kl_node_130) / kl_node_130.shape[0]
            kl_7 = torch.sum(kl_node_7) / kl_node_7.shape[0]
            kl_loss = kl_7 + kl_130
            # calculate the disentangle loss
            feat_node_130 = th.cat(feat_node_130, dim=0)
            feat_others_130 = th.cat(feat_others_130, dim=0)
            l_clr = contrastive_loss(feat_node_7, feat_node_130, options.con_temp, device,
                                     normalization=options.norm_clr)
            ...
            if kl_flag:
                total_loss += options.weight_kl * kl_loss
            ...
            print("e{}, sampled 130 designs: {}, b{}/{}, total loss:{:.2f}, 7nm reg loss: {:.2f}, 130nm reg loss: {}, "
                  "kl 7nm: {:.6f}, kl 130nm: {:.6f}, clr loss: {:.2f}, cmd loss: {:.2f}, 7nm r2:{:.2f}, 130nm r2: {}, batch time: {:.2f}".format(epoch,
                                                                                                             designs_info,
                                                                                                             bidx,
                                                                                                             num_batch_7,
                                                                                                             total_loss.item(),
