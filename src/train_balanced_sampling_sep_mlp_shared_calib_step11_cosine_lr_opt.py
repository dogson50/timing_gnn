r"""Step11: Cosine annealing LR scheduler.

Key idea: Cosine annealing avoids late-stage oscillation from a fixed LR,
allowing the model to converge more smoothly. Combined with warm restarts
for potential escape from local minima.
"""
...
def train_step11(options, seed):
    ...
    scheduler = th.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, T_0=max(1, options.num_epoch // 3), T_mult=2, eta_min=1e-6
    )
    print(f"[Step11] CosineAnnealingWarmRestarts: T_0={options.num_epoch // 3}, T_mult=2, eta_min=1e-6")
    ...
    for epoch in range(options.num_epoch):
        ...
        print(f"e{epoch}, train_loss:{train_loss:.4f}, r2:{train_r2:.3f}, "
              f"val_loss:{val_loss:.4f}, val_r2:{val_r2:.3f}, lr:{cur_lr:.2e}, src_w:{src_w:.4f}")
        ...
        if better_r2 or tie_better:
            ...
            th.save({"enc": enc.state_dict(), "model": model.state_dict(),
                      ...
                      "script": "step11_cosine_lr"}, best_ckpt_path)
...
if __name__ == "__main__":
    ...
    with tee.StdoutTee(stdout_f), tee.StderrTee(stderr_f), tee.StdoutTee(copilot_log_f), tee.StderrTee(copilot_log_f):
        run_train_and_report_test(train_step11, options, seed, script_name=__file__)