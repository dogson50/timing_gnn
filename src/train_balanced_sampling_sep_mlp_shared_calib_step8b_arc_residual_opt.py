r"""Step8b: Step5-A baseline + residual arc conditioning for freeze stability."""
...
class SharedCalibArcRegressor(nn.Module):
    def __init__(
        ...
        *,
        ...
        if self.arc_cond_mode not in ("concat", "film", "residual"):
        ...
        if self.use_arc_cond:
            ...
            if self.arc_cond_mode == "residual":
                self.arc_proj_tgt = nn.Sequential(
                    nn.Linear(self.arc_dim, self.base_dim),
                    nn.LayerNorm(self.base_dim),
                    nn.Tanh(),
                )
                self.arc_proj_src = nn.Sequential(
                    nn.Linear(self.arc_dim, self.base_dim),
                    nn.LayerNorm(self.base_dim),
                    nn.Tanh(),
                )
                # Near-zero init keeps early training close to Step5-A.
                self.arc_gate_tgt = nn.Parameter(th.tensor(0.05, dtype=th.float32))
                self.arc_gate_src = nn.Parameter(th.tensor(0.02, dtype=th.float32))
    ...
    def forward(self, x, z, node="tgt", from_pin_id=None, to_pin_id=None, pol_id=None):
        ...
        if apply_arc:
            ...
            if self.arc_cond_mode == "concat":
                ...
                if node == "tgt":
                    delta = self.arc_proj_tgt(arc_feat)
                    gate = self.arc_gate_tgt
                elif node == "src":
                    delta = self.arc_proj_src(arc_feat)
                    gate = self.arc_gate_src
                else:
                    raise ValueError(f"Unknown node type: {node}")
                h = h_base + gate * delta
...
def train_balanced_sep_mlp_shared_calib(options, seed):
    ...
    use_arc_cond = bool(getattr(options, "use_arc_cond", False))
    arc_cond_mode = str(getattr(options, "arc_cond_mode", "residual")).lower()
    if use_arc_cond and arc_cond_mode in ("concat", "film"):
        print(
            f"[Info] Step8b recommends --arc_cond_mode residual for freeze stability; current={arc_cond_mode}"
        )
    src_use_arc_cond = not bool(getattr(options, "disable_src_arc_cond", False))
    if use_arc_cond and options.freeze_hgat and src_use_arc_cond:
        # In freeze mode, source arc branch often adds noise; disable by default in this variant.
        src_use_arc_cond = False
        print("[Info] freeze_hgat=True -> forcing src arc conditioning OFF in Step8b")
    ...
    model = SharedCalibArcRegressor(
        ...
        use_arc_cond=use_arc_cond,
        arc_sep_domain_emb=getattr(options, "arc_sep_domain_emb", False),
        arc_cond_mode=arc_cond_mode,
        src_use_arc_cond=src_use_arc_cond,
    ).to(device)
            ...
            th.save(
                {
                    ...
                    "arc_sep_domain_emb": getattr(options, "arc_sep_domain_emb", False),
                    "arc_cond_mode": arc_cond_mode,
                    "disable_src_arc_cond": (not src_use_arc_cond),
                    "scaler_stats": scaler_stats,
                    "y_scaler": y_scaler,
                    "epoch": epoch + 1,
                    "best_val_r2": best_val,
                    "src_loss_weight_epoch": src_weight_epoch,
                    "script": "shared_calib_step8b_arc_residual_opt",
                },
                best_ckpt_path,
            )
