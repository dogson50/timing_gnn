r"""Balanced sampling + disentangle (cell_type CLR, optional CMD on process) with HGAT embeddings."""
        ...
        for xb_tgt, yb_tgt, cts_tgt in dl_tgt:
            ...
            for _ in range(max(1, options.sample_45_num)):
                ...
                if options.weight_clr > 0.0:
                    clr_feats = th.cat([h_design_tgt, h_design_src], dim=0)
                    clr_labels = list(cts_tgt) + list(cts_src)
                    clr_list.append(
                ...
                if options.weight_cmd > 0.0:
                    cmd_list.append(cmd_loss(h_process_tgt, h_process_src, k=options.cmd_k))
