...
def infer_output_pin_strict(top_pins):
    pins_up = [str(p).upper() for p in (top_pins or [])]
    output_name_set = {"Y", "Z", "ZN", "Q", "QN"}
    for p in pins_up:
...
def build_dgl_graph_from_devs_step5(devs, top_pins):
    ...
    output_pin = infer_output_pin_strict(top_pins)
    ...
    for name, nid in sorted(nets.items(), key=lambda x: x[1]):
        ...
        is_output_pin = 1.0 if (output_pin is not None and up == output_pin) else 0.0
        ...
        f_net.append([is_vdd, is_vss, is_top_pin, is_output_pin, deg_norm, gate_norm, sd_norm, is_internal])
