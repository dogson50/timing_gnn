...
class SharedCalibRegressor(nn.Module):
    def __init__(
        ...
        fusion_gate_hidden=0,
        fusion_gate_mode="channel",
        fusion_gate_dropout=-1.0,
    ):
        super().__init__()
        self.use_gated_fusion = bool(use_gated_fusion)
        self.fusion_gate_mode = str(fusion_gate_mode).lower()
        if self.fusion_gate_mode not in ("scalar", "channel"):
            raise ValueError(f"Unsupported fusion_gate_mode={fusion_gate_mode}")
        if self.use_gated_fusion:
            gate_hid = int(fusion_gate_hidden) if int(fusion_gate_hidden) > 0 else int(hid)
            gate_drop = float(fusion_gate_dropout) if float(fusion_gate_dropout) >= 0 else float(dropout)
            gate_out = 1 if self.fusion_gate_mode == "scalar" else int(hid)
            ...
            self.fusion_gate = nn.Sequential(
                nn.Linear(in_dim + design_dim, gate_hid),
                nn.ReLU(),
                nn.Dropout(gate_drop),
                nn.Linear(gate_hid, gate_out),
                nn.Sigmoid(),
            )
    ...
    def forward(self, x, z, node="tgt", topo_ids=None):
        ...
        xz = th.cat([x, z], dim=1)
        if self.use_gated_fusion:
            h_num = self.num_branch(x)
            h_graph = self.graph_branch(z)
            gate = self.fusion_gate(xz)
            h = gate * h_graph + (1.0 - gate) * h_num
            h = self.fusion_refine(h)
        else:
            h = self.backbone(xz)
...
def train_balanced_sep_mlp_shared_calib(options, seed):
    ...
    use_gated_fusion = bool(getattr(options, "use_gated_fusion", False))
    fusion_gate_hidden = int(getattr(options, "fusion_gate_hidden", 0))
    fusion_gate_mode = str(getattr(options, "fusion_gate_mode", "channel")).strip().lower()
    fusion_gate_dropout = float(getattr(options, "fusion_gate_dropout", -1.0))
    ...
    print(
        f"[Info] Gated fusion: {'enabled' if use_gated_fusion else 'disabled'}"
        + (
            f" (mode={fusion_gate_mode}, gate_hidden={fusion_gate_hidden if fusion_gate_hidden > 0 else hgat_hid}, "
            f"gate_dropout={fusion_gate_dropout if fusion_gate_dropout >= 0 else dropout})"
            if use_gated_fusion else ""
        )
    )
