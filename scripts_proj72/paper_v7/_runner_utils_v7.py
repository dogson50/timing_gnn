...
def run_v7_search(
    *,
    ...
    train_py = project_root / "src" / "train_balanced_sampling_sep_mlp_shared_calib_step5a_feat_v7_opt.py"
    ...
    top_names = [x["candidate"] for x in quick_summary[:max(1, int(top_k_expand))]]
    print(f"[ExpandTop{len(top_names)}] {top_names}")
