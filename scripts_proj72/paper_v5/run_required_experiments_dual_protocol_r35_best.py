...
def prepare_celltype_datasets(project_root: Path, py: Path):
    ...
    prep_py = (
        ...
        / "scripts_proj72"
        / "paper_v5"
        / "prepare_celltype_train_val_no_test_dataset.py"
    )
    sup_py = (
        project_root
        / "scripts_proj72"
        / "paper_v5"
        / "build_supervision_ratio_datasets_generic.py"
    )
    ...
    if not cell_pkl.exists():
        subprocess.check_call(
            [
                ...
                str(data_dir),
                "--out_name",
                "dataset_cell_type_train_val_no_test.pkl",
                "--split_seed",
                "42",
                "--train_ratio",
                str(1.0 / 6.0),
    ...
    # Keep this utility import reachable for compatibility, but do not generate
    # row-level supervision-ratio datasets in this queue.
    _ = sup_py


def prepare_table_group_group_ratio_datasets(project_root: Path, py: Path):
    data_dir = project_root / "ICCAD2026_Changxin" / "paper_materials" / "data"
    ensure_dirs(data_dir)
    base_pkl = data_dir / "dataset_table_group_train_val_no_test.pkl"
    group_ratio_py = (
        ...
        / "scripts_proj72"
        / "paper_v5"
        / "build_group_level_ratio_datasets.py"
    )
    subprocess.check_call(
        [
            str(py),
            ...
            str(data_dir),
            "--prefix",
            "dataset_table_group_group_ratio_val50_no_test",
            "--group_col",
            "group_id",
            "--seed",
            "9294",
...
def build_protocols() -> Dict[str, dict]:
    # Protocol-A: best verified TCDP variant line (round35 r35_02).
    tg_common = [
        "--freeze_hgat",
        "--num_epoch",
        "320",
    ...
    # Keep this only for TCDP joint full-setting to match round35 anchor,
    # but never apply it to target-only/source-only/ablation runs.
    tg_full_joint_extra = [
    ...
    def tg_ratio_extra(tag: str):
        # group-level ratio tags: g100/g050/g033/g020/g010
        if tag == "g020":
            return [*tg_ratio_base, "--tgt_group_reweight", "--tgt_group_reweight_power", "0.2"]
        return list(tg_ratio_base)

    # Protocol-B: historical cell_type best-line.
    ct_common = [
        "--freeze_hgat",
        "--num_epoch",
        "240",
    ...
    return {
        "tg_best_r35_02_fix1": {
            ...
            "ratio_prefix": "dataset_table_group_group_ratio_val50_no_test",
            "ratio_tags": ["g100", "g050", "g033", "g020", "g010"],
            "common_args": tg_common,
            "full_joint_extra": tg_full_joint_extra,
...
def main():
    ...
    train_step5a = (
        project_root / "src" / "train_balanced_sampling_sep_mlp_shared_calib_step5a_feat_opt.py"
    )
    parse_py = project_root / "scripts_proj72" / "paper_v3" / "parse_experiment_logs.py"

    logs_dir = project_root / "ICCAD2026_Changxin" / "paper_materials" / "logs_ext_v5_r35_group_ratio"
    tables_dir = project_root / "ICCAD2026_Changxin" / "paper_materials" / "tables_ext_v5_r35_group_ratio"
    ensure_dirs(logs_dir, tables_dir)

    log_monitor(
        project_root,
        "continue: start v5-r35-group-ratio experiments (group-level target/source ratio study)",
    )

    prepare_celltype_datasets(project_root, py)
    prepare_table_group_group_ratio_datasets(project_root, py)
    ...
    def add_run(
        ...
        model_dir = f"model_paperv5r35_{run_id}"
        log_path = logs_dir / f"{run_id}.log"
        cmd = (
            [str(py), str(train_py)]
            + build_common_args(model_dir, dataset_pkl, seed, cfg)
            + list(args_extra)
        )
        runs.append(
            RunItem(
    ...
    for protocol, cfg in protocols.items():
        ...
        ratio_tags = cfg.get("ratio_tags", [])

        for s in seeds:
            add_run(protocol, "tcdp_joint", s, base_ds, list(cfg.get("full_joint_extra", [])))
            add_run(
                protocol,
                "target_only",
                s,
            ...
            add_run(
                ...
                s,
                base_ds,
                ["--target_loss_weight", "0.0", "--loss_weight_45", "1.0"],
            )

            pre_exp = "pretrain_stage1_source_only"
            pre_ds = base_ds
            pre_model_dir = (
                f"model_paperv5r35_{protocol}__{pre_exp}_s{s}_{pre_ds.replace('.pkl', '')}"
            )
            ...
            add_run(
                ...
                s,
                pre_ds,
                ["--target_loss_weight", "0.0", "--loss_weight_45", "1.0", "--num_epoch", "140"],
            )
            add_run(
                protocol,
                "pretrain_finetune",
            ...
            add_run(
                ...
                s,
                base_ds,
                maybe_remove_topology([]),
            )

            for tag in ratio_tags:
                ds = f"{ratio_prefix}_{tag}.pkl"
    ...
    for r in runs:
        ...
        if prev is not None and prev.get("status") == "done" and is_log_finished(
            Path(prev.get("log_path", ""))
        ):
            row.update(prev)
        all_rows.append(row)
    ...
    for row in all_rows:
        ...
        log_monitor(project_root, f"v5-r35 running: {row['run_id']}")
    ...
    log_monitor(project_root, "v5-r35 queue finished: all dual-protocol runs completed")
